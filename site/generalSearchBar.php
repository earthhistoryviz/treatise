...
// Get the genus name from the server domain
$siteGenus = ucfirst(strtolower($_SERVER['SERVER_NAME']));
// Remove any domain suffix if needed (e.g., "brachiopod.treatise.geolex.org" -> "brachiopod")
$siteGenus = explode('.', $siteGenus)[0];
	...
	<div class="row justify-content-center">
    	<div class="col-md-8">
		  <div class="title-section text-center mb-4">
			<h2><?php if (isset($_GET['search']) && !empty(trim($_GET['search']))) { echo 'Welcome to the <strong>' . htmlspecialchars(ucfirst(trim($_GET['search']))) . '</strong> on Invertebrate Paleontology!'; } else { echo 'Welcome to the <strong>' . htmlspecialchars($siteGenus) . '</strong> subsite on Invertebrate Paleontology!'; } ?></h2>
