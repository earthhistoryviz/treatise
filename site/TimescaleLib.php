...
function parseGeologicalStages($filepath) {
  $COLPERIOD=0;
  $COLSTAGE=1;
  $COLBASE=4;
  $COLTOP=5;
  $COLBEGUP=10;
  $COLBASEINTERNATIONAL=11;
  $COLENDUP=16;
  $COLTOPINTERNATIONAL=17;
  $xlsx = SimpleXLSX::parse($filepath);
  if (!$xlsx) throw new RuntimeException("XLSX Parse Error for path $filepath: ". SimpleXLSX::parseError());

  // Find the MasterChronostrat sheet:
  $sheetnames = $xlsx->sheetNames();
  $mastersheetindex = array_search('Geological stages', $sheetnames);
  if ($mastersheetindex < 0) throw new RuntimeException("Could not find Geological stages sheet");
  ...
  for($i = 3; $i < count($rows); $i++) {
    $row = $rows[$i];
    $stages[$row[$COLSTAGE]] = array(
      "period" => $row[$COLPERIOD],
      "stage" => $row[$COLSTAGE],
      "base" => $row[$COLBASE],
      "top" => $row[$COLTOP],
      "begin_percent_up" => $row[$COLBEGUP],
      "international_base" => $row[$COLBASEINTERNATIONAL],
      "end_percent_up" => $row[$COLENDUP],
      "international_top" => $row[$COLTOPINTERNATIONAL]
    );
  }
